...
# NOTE: THE TOKEN IS GOT FROM REQUEST OVER HERE, HAVE IT GET THE COOKIE
def get_http_authorization(request):
    ...
    # if len(auth) != 2 or auth[0].lower() != prefix.lower():
        # print('cookie:', request.COOKIES.get(jwt_settings.JWT_COOKIE_NAME))
    return request.COOKIES.get(jwt_settings.JWT_COOKIE_NAME)
    # return auth[1]

def get_http_refresh_authorization(request):
    return request.COOKIES.get(jwt_settings.JWT_REFRESH_TOKEN_COOKIE_NAME)
