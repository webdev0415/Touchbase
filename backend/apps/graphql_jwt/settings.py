...
DEFAULTS = {
    ...
    'JWT_ENCODE_HANDLER': 'apps.graphql_jwt.utils.jwt_encode',
    'JWT_DECODE_HANDLER': 'apps.graphql_jwt.utils.jwt_decode',
    'JWT_PAYLOAD_HANDLER': 'apps.graphql_jwt.utils.jwt_payload',
    'JWT_PAYLOAD_GET_USERNAME_HANDLER': (
        lambda payload: payload.get(get_user_model().USERNAME_FIELD)
    ),
    'JWT_GET_USER_BY_NATURAL_KEY_HANDLER':
    'apps.graphql_jwt.utils.get_user_by_natural_key',
    'JWT_REFRESH_EXPIRED_HANDLER': 'apps.graphql_jwt.utils.refresh_has_expired',
    'JWT_GET_REFRESH_TOKEN_HANDLER':
    'apps.graphql_jwt.refresh_token.utils.get_refresh_token_by_model',
    'JWT_ALLOW_ANY_HANDLER': 'apps.graphql_jwt.middleware.allow_any',
...
class JWTSettings(object):
    ...
    def __getattr__(self, attr):
        ...
        if attr == 'JWT_ALLOW_ANY_CLASSES':
            value = list(value) + [
                'apps.graphql_jwt.mixins.JSONWebTokenMixin',
                'apps.graphql_jwt.mixins.VerifyMixin',
                'apps.graphql_jwt.refresh_token.mixins.RevokeMixin',
            ]
